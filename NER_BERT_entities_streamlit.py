...
linking, and visualization using dslim/bert-large-NER.

Author: Based on entity_linker.py
Version: 1.1 - Updated to use BERT instead of Flair
"""
...
st.set_page_config(
    page_title="From Text to Linked Data using BERT",
    layout="centered",  
...
class EntityLinker:
    ...
    extraction, validation, linking, and output generation using BERT NER.
    """
    
    def __init__(self):
        """Initialize the EntityLinker and load required BERT model."""
        self.ner_pipeline = self._load_bert_model()
    ...
    def _load_bert_model(self):
        """Load BERT NER model with error handling."""
        try:
    ...
    def extract_entities(self, text: str):
        """Extract named entities from text using BERT NER with proper validation."""
        ...
        for ent in ner_results:
            # Filter out unwanted entity types and low confidence predictions
            if ent['score'] < 0.5:  # Skip low confidence predictions
                continue
            ...
            # Map BERT entity types to our format
            entity_type = self._map_bert_entity_type(tag)
            ...
            if self._is_valid_entity(ent['word'], entity_type, ent):
                entities.append({
                    'text': ent['word'],
                    'type': entity_type,
                    'start': ent['start'],
                    'end': ent['end'],
                    'label': tag,  # Keep original BERT label for reference
    ...
    def _map_bert_entity_type(self, bert_label: str) -> str:
        """Map BERT entity labels to our standardized types."""
        # dslim/bert-large-NER uses CoNLL03 tags: PER, ORG, LOC, MISC
        mapping = {
            'PER': 'PERSON',
            'ORG': 'ORGANIZATION',
            'LOC': 'LOCATION',
            'MISC': 'MISC',
            # Handle B- and I- prefixes if aggregation_strategy doesn't work
            'B-PER': 'PERSON',
            'I-PER': 'PERSON',
            'B-ORG': 'ORGANIZATION',
            'I-ORG': 'ORGANIZATION',
    ...
    def _is_valid_entity(self, entity_text: str, entity_type: str, bert_result) -> bool:
        """Validate an entity from BERT NER."""
        ...
        # For BERT, we have confidence scores, so we can use those
        if bert_result.get('score', 0) < 0.5:
            return False
        ...
        if entity_type == 'PERSON':
            ...
            if len(words) > 0 and all(word[0].isupper() for word in words if word.isalpha()):
                return True
            # Also accept single capitalized words that could be names
            if len(words) == 1 and entity_text[0].isupper() and entity_text.isalpha():
                return True
            return False
            
        # For other types, we'll trust BERT's prediction if confidence is high enough
        return bert_result.get('score', 0) >= 0.7
    ...
    def _remove_overlapping_and_duplicate_entities(self, entities):
        ...
        # Step 2: Handle overlapping entities (keep longer ones, or higher confidence ones)
        entities.sort(key=lambda x: x['start'])
...
class StreamlitEntityLinker:
    ...
    def render_header(self):
        ...
        st.header("From Text to Linked Data using BERT")
        ...
        <div style="background-color: white; padding: 20px; border-radius: 10px; margin: 20px 0; border: 1px solid #E0D7C0;">
            <div style="text-align: center; margin-bottom: 20px;">
                ...
                <div style="background-color: #9fd2cd; padding: 10px; border-radius: 5px; display: inline-block; margin: 5px;">
                     <strong>BERT Entity Recognition</strong>
                </div>
                <div style="margin: 10px 0;">⬇️</div>
    ...
    def process_text(self, text: str, title: str):
        ...
        with st.spinner("Processing text and extracting entities..."):
            try:
                ...
                status_text.text("Extracting entities with BERT...")
    ...
    def create_highlighted_html(self, text: str, entities: List[Dict[str, Any]]) -> str:
        ...
        # Color scheme (updated for BERT entity types)
        colors = {
        ...
        for entity in sorted_entities:
            ...
            if entity.get('confidence'):
                tooltip_parts.append(f"Confidence: {entity['confidence']:.2f}")
    ...
    def render_entity_table(self, entities: List[Dict[str, Any]]):
        ...
        for entity in entities:
            ...
            if entity.get('confidence'):
                row['Confidence'] = f"{entity['confidence']:.2f}"
            ...
            if st.session_state.html_content:
                ...
                <body>
                    <div class="header">
                        ...
                        <p>Found {len(entities)} entities using BERT NER</p>
                    </div>
                    <div class="content">
                        {st.session_state.html_content}
                    </div>
